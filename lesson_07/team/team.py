...
forks = [threading.Lock() for _ in range(FORKS)]
meals_by_philosopher = [0] * PHILOSOPHERS
total_meals_lock = threading.Lock()
total_meals_eaten = 0

room_semaphore = threading.Semaphore(PHILOSOPHERS - 1)

def eating(philosopher_id):
    global total_meals_eaten

    left = philosopher_id
    right = (philosopher_id + 1) % PHILOSOPHERS
    ...
    while True:
        ...
        with total_meals_lock:
            if total_meals_eaten >= MAX_MEALS_EATEN:
                forks[right].release()
                forks[left].release()
            ...
            total_meals_eaten += 1
            meals_by_philosopher[philosopher_id] += 1
            print(f"Philosopher {philosopher_id} is eating. Total meals: {total_meals_eaten}")
...
def main():
    ...
    for i in range(PHILOSOPHERS):
        p = threading.Thread(target=eating, args=(i,))
    ...
    print("\n--- Dining Complete ---")
    for i in range(PHILOSOPHERS):
        print(f"Philosopher {i} ate {meals_by_philosopher[i]} times.")
